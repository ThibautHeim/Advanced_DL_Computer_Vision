...
class MaskedAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_logits = torch.matmul(queries,torch.transpose(keys,1,2))
        attention_logits = attention_logits / math.sqrt(embed_dim) + torch.ones((batch_size*self.num_heads,seq_length,seq_length))
        ...
        mask = torch.triu(torch.full((seq_length,seq_length), True),diagonal=1)
